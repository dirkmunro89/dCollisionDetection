...
#
#   q=np.append(np.zeros(nut), np.array([2.,2.,2.]))
...
#
#   obj.setup(H,q,A,l,u,verbose=True,eps_abs=1e-12,eps_rel=1e-12,max_iter=int(1e6),polish=True)
    obj.setup(H,q,A,l,u,verbose=False,eps_abs=1e-12,eps_rel=1e-12)#,max_iter=int(1e6),polish=True)
#   obj.setup(sparse.csc_matrix(ddg),dg[0],A,l,u,verbose=False,eps_abs=1e-12,eps_rel=1e-12)
#
    sol=obj.solve()
#
    xt=sol.x[:nut]
